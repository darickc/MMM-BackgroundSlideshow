...
module.exports = NodeHelper.create({
  ...
  excludedFiles(currentDir) {
    try {
      ...
      Log.info(
        `Found excluded images list: in dir: ${currentDir} containing: ${listOfExcludedFiles.length} files`,
      );
  ...
  isExcluded(filename, excludedImagesList) {
    if (
      excludedImagesList.includes(filename.replace(/\.[a-zA-Z]{3,4}$/u, ""))
    ) {
  ...
  readEntireShownFile() {
    ...
    try {
      ...
      Log.info(
        `Found filesShownTracker: in path: ${filepath} containing: ${listOfShownFiles.length} files`,
      );
  ...
  gatherImageList(config, sendNotification) {
    ...
    Log.info(
      `Skipped ${this.imageList.length - imageListToUse.length} files since already seen!`,
    );
    let finalImageList;
  ...
  getNextImage() {
    ...
    try {
      if (fs.existsSync(imageDirectory)) {
        ...
        match = imageFilename
          .replace(/\.[^.]+$/, "")
          .match(/^(.+?)\s*\((\d+)\)$/);
        let baseFilename = match
          ? match[1]
          : imageFilename.replace(/\.[^.]+$/, "");
        let imageFilenameEnumeratedEnd = match ? `(${match[2]})` : "";

        filesArray = files.filter((file) => {
          return (
            file.startsWith(baseFilename) &&
            path.extname(file).toLowerCase() ===
              imageFilenameEnumeratedEnd + ".json"
          );
        });
    ...
    if (jsonFilePath !== "") {
      if (
        ...
        !this.config.excludeDescriptionsRegexps?.some((regexp) =>
          new RegExp(regexp).test(json_metadata.description),
        )
      ) {
      ...
      if (
        json_metadata.photoTakenTime &&
        json_metadata.photoTakenTime.timestamp
      ) {
        ...
        metadata.photoTakenTime = dateTime
          .toISOString()
          .slice(0, 16)
          .replace("T", " ");
        Log.debug(
        ...
        json_metadata.creationTime.timestamp
      ) {
  ...
  getFiles(imagePath, imageList, excludedImagesList, config) {
    ...
    Log.info(
      `Reading directory "${imagePath}" for images, found ${contents.length} files and directories`,
    );
  ...
  signalPhoto(payload) {
    ...
    Log.log(
      `Signaling photo: ${payload.path} on URL: ${this.config.photoSignalUrl}`,
    );
